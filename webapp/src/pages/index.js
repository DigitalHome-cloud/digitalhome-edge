...
// Native edge UI (no iframe): Gatsby/React shell, live data via WebSocket hook,
// onboarding via POST /app-api/scan + /app-api/connect.
...
function useEdgeStream() {
  ...
  useEffect(() => {
    ...
    const startPoll = () => {
      ...
      const p = async () => { try { setData(await (await fetch("/app-api/status", { cache: "no-store" })).json()); } catch { setLive(false); } };
      p(); pollRef.current = setInterval(p, 3000);
    };
...
const api = (path, body) =>
  fetch(`/app-api/${path}`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }).then((r) => r.json());

function Tile({ title, link, rows }) {
  const on = link && link.state === "linked";
  return (
    <div className="card">
      <h2>{title}<span className={"badge" + (on ? " ok" : "")}>{on ? "connected" : (link && link.state) || "—"}</span></h2>
      {rows.map(([k, v]) => (<div className="kv" key={k}><span className="k">{k}</span><span className="v">{fmt(v)}</span></div>))}
    </div>
  );
}

function Msg({ m }) {
  if (!m) return <div className="msg" />;
  return <div className={"msg " + (m.cls || "")}>{m.text}</div>;
}
...
function Onboard({ source, link, children, onConnect, connectLabel }) {
  ...
  return (
    <div className="card">
      <h2>{{ solar: "Solar (Solarman)", ccu: "Homematic CCU", hue: "Philips Hue" }[source]}
        <span className={"badge" + (on ? " ok" : "")}>{on ? "connected" : (link && link.state) || "—"}</span></h2>
      {children}
      <button onClick={onConnect}>{connectLabel || "Connect"}</button>
    </div>
  );
}
...
export default function Home() {
  ...
  const scan = async (source) => {
    say(source, "scanning the network…", "busy");
    try {
      const j = await api("scan", { source });
      if (j.candidates && j.candidates.length) {
        setF((x) => ({ ...x, [source + "Ip"]: j.candidates[0].ip }));
        say(source, "found: " + j.candidates.map((c) => c.ip).join(", "), "ok");
      } else say(source, `no ${source.toUpperCase()} found on ${j.subnet || "?"}.0/24`, "err");
    } catch { say(source, "scan failed", "err"); }
  };
  const connect = async (source) => {
    ...
    try {
      const j = await api("connect", body);
      say(source, (j.ok ? "✓ " : "✗ ") + (j.message || (j.ok ? "connected" : "failed")), j.ok ? "ok" : "err");
      if (source === "solar") setF((x) => ({ ...x, solarPass: "" }));
    } catch { say(source, "request failed", "err"); }
  };
      ...
      <main>
        ...
        <div className="grid">
          <Onboard source="solar" link={solar.link} onConnect={() => connect("solar")}>
            ...
            <input type="password" placeholder="Solarman password" value={f.solarPass} onChange={set("solarPass")} />
            <Msg m={msg.solar} />
          </Onboard>
          <Onboard source="ccu" link={ccu.link} onConnect={() => connect("ccu")}>
            ...
            <input placeholder="API token (recommended)" value={f.ccuToken} onChange={set("ccuToken")} />
            <Msg m={msg.ccu} />
          </Onboard>
          <Onboard source="hue" link={hue.link} onConnect={() => connect("hue")} connectLabel="Connect (press bridge button)">
            ...
            <input placeholder="Hue bridge IP" value={f.hueIp} onChange={set("hueIp")} />
            <Msg m={msg.hue} />
          </Onboard>
        </div>
      </main>
      <footer>Native React shell · live over WebSocket · onboarding via /app-api/* — no iframe</footer>
      <style>{css}</style>
    </>
  );
}
...
const css = `
  ...
  button{padding:9px 14px;border:0;border-radius:8px;background:#2563eb;color:#fff;font-size:14px;cursor:pointer;margin:4px 6px 4px 0} button.ghost{background:#334155}
